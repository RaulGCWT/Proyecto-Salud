...
from database import table_invites, table_family_users
...
def sync_invitation_status(invitation, persist=True):
    ...
    current_state = invitation.get('state') or INVITE_PENDING
...
def serialize_invitation(invitation, persist=True):
    synced = sync_invitation_status(dict(invitation), persist=persist)
    if not synced:
        return None
...
def change_invitation_state(invitation, new_state):
    ...
    append_invite_history(invitation, new_state, changed_at)
    table_invites.put_item(Item=invitation)
    return serialize_invitation(invitation, persist=False)
