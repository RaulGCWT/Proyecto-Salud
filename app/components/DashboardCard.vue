...
<style scoped>
.card {position: relative;overflow: hidden;padding: 24px;border-radius: 16px;border-left: 5px solid #cbd5e1;background: white;box-shadow: 0 4px 6px rgba(0, 0, 0, 0.05);transition: all 0.3s;}
...
.card-value {margin: 8px 0;font-size: 2rem;font-weight: 800;}
.heart-color { color: #ef4444; }
.hrv-color { color: #06b6d4; }
.resp-color { color: #8b5cf6; }
.presence-color { color: #10b981; }
.card-indicator {display: inline-block;padding: 4px 10px;border-radius: 20px;font-size: 0.85rem;font-weight: 600;}
.status-ok {color: #166534;background: #dcfce7;}
.status-error {color: #991b1b;background: #fee2e2;}
.card-alert {border-left-color: #b91c1c !important;animation: pulse-red 1.5s infinite;}

@keyframes pulse-red {
  0% {box-shadow: 0 0 0 0 rgba(239, 68, 68, 0.4);background-color: white;}
  50% {box-shadow: 0 0 0 15px rgba(239, 68, 68, 0);background-color: #fef2f2;}
  100% {box-shadow: 0 0 0 0 rgba(239, 68, 68, 0);background-color: white;}
}
.is-empty {background-color: #f1f5f9 !important;border-left-color: #94a3b8 !important;opacity: 0.8;}
